...
For website browsing, interaction, screenshots, and live frontend verification, pstack selects a driver through [browse-web](./skills/browse-web/SKILL.md). Codex uses a callable Computer Use runtime when the current session exposes one, whether or not its companion skill appears in the skill catalog; otherwise it reports the concrete runtime failure and falls back to the installed [Vercel `agent-browser` CLI](https://agent-browser.dev/). Other harnesses use `agent-browser`. Explicit user tool requirements take precedence. For the CLI route, run `agent-browser --version` and `agent-browser doctor` to check the prerequisite. Existing automated test suites keep their runners.

new here? the [pstack guide](./docs/guide/README.md) walks you through a first real task, from setup and prompting through verification and overnight runs.

that's it. the other skills are situational. the mode uses them as needed. model roles resolve through `harness/<yours>.md`. cursor follows upstream's fable 5.1 / sol / grok / opus 5 panel. codex inherits the parent model unless a supported role override is configured. [`/setup-pstack`](./skills/setup-pstack/SKILL.md) changes those choices.
...
| [test-where-the-failure-happens](./skills/principle-test-where-the-failure-happens/SKILL.md) | verification | Plan Given/When/Then scenarios, test UI in a real browser, and share page objects and data factories across browser runners. |
...
| [register-cleanup-before-work](./skills/principle-register-cleanup-before-work/SKILL.md) | verification | When a test acquires a resource, bind or register its cleanup immediately, before assertions or any other work that can throw. |
