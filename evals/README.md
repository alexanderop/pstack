...
Run real Codex behavior through Vitest 5 before changing the plugin. The suites distinguish installation, runtime behavior, and application outcomes.
...
`pnpm test` runs helper regressions, unit tests, and installation-contract tests without model turns. `pnpm eval:list` lists cases without executing them. `pnpm eval:codex` starts a real native-child smoke test. `pnpm eval:baseline` runs the actual installed comment-review skill and audits a worktree with a recent real Codex session. Live commands use your existing Codex login.
...
The historical first baseline was red for cleanup; the current audit now includes Codex session lookup. A clean, merged worktree is read by real Codex before running the unchanged installed audit script. The assertion requires `verify-recent-chat`; the old script returned `safe`. Comment review is invoked naturally, without adapter instructions in the test prompt. It passed one completed baseline and failed a later trial because the child did not load the Comment Sicko wrapper, although the output file was correct. Preserve both observations; do not retry away the missing-instruction failure or demand a particular agent name.
...
The cleanup implementation now has deterministic Codex, Claude Code, and Cursor regression fixtures. The existing live acceptance assertion remains unchanged; a fresh live run is needed to update the historical baseline. Do not mark the test as an expected failure, weaken its assertion, or retry until it passes.
