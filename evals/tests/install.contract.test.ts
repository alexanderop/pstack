...
test('a clean Codex installation discovers the bundled skills without user configuration', async ({ signal, annotate }) => {
  ...
  try {
    ...
    const browserSkill = skills.find(skill => skill.name === 'pstack:browse-web')
    expect(browserSkill).toBeDefined()
    if (browserSkill) {
      expect(await readFile(join(dirname(browserSkill.path), 'agents/openai.yaml'), 'utf8'))
    ...
    for (const skill of skills.filter(skill => !['pstack:setup-pstack', 'pstack:browse-web'].includes(skill.name))) {
