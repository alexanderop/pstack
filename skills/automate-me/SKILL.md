...
This skill orchestrates three others: an inline mining pass (see step 1), the harness's skill-authoring tool (`harness/built-ins.md`), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `**/*-mode/SKILL.md` under the active harness's project-local and user-level skill paths (`harness/<detected>.md` names both) matching the user's handle. Mode skills can live in a personal category directory (`<skill-path>/<handle>/`), not only at the top level. If one exists, confirm intent with `AskQuestion` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out, using the "Transcripts" section of `harness/<detected>.md` — the location and format differ per harness, and two of the four are not a per-workspace directory at all. Scope to the current workspace only. Never glob across all workspaces; that reads private chats from unrelated projects.
...
Use the harness's skill-authoring tool (`harness/built-ins.md`) to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use `<project-skill-path>/<handle>/<handle>-mode/SKILL.md` when the repo has an established personal category for that handle; otherwise default to `<project-skill-path>/<handle>-mode/SKILL.md` (or the user-level skill path if the user prefers a personal skill). Both paths are in `harness/<detected>.md`.
...
- The harness's skill-authoring tool (`harness/built-ins.md`): skill authoring process and writing guidelines.
