...
- **Browser setup:** for web surfaces, include the `agent-browser` prerequisite, a unique named session passed to every browser command, snapshot/semantic interaction commands, screenshot inspection, and session cleanup. Put the concrete commands in the generated skill so it remains usable without pstack installed.
