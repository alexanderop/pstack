...
Transcripts live somewhere different on every harness, and two of the four are
not a directory of per-workspace jsonl files at all. Read the "Transcripts"
section of the active harness file (`harness/<detected>.md`; `scripts/detect-harness.sh`
names it) before you search. The shapes you will meet:

- **Cursor** and **Claude Code** — a per-workspace directory of jsonl files, one
  chat message per line. The slug rules differ by one leading dash; the harness
  file has the exact form.
- **Codex** — jsonl filed by *date*, not by workspace. Bound the search by date
  first, then match each candidate's cwd.
- **Copilot CLI** — a sqlite index plus per-session jsonl. Query
  `session-store.db` by `cwd` for the session ids, then read each session's
  `events.jsonl`.

Never glob across all workspaces. That crosses into private chats from unrelated
projects.
