...
This workflow requires Cursor's Grok Bot routines (`update_state`) and secret-request cards (`SendToUser`). Check that those tools are available before following the steps below. Codex does not gain them by installing this plugin. If they are absent, build the page and server against a user-supplied webhook URL, keep credentials in server-side configuration, and report that routine creation and the live wake remain unavailable. Do not invent tool calls, request secrets in chat, or claim the bot is connected without a successful probe.
