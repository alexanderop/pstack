...
**Five shapes to inspect for missing behavioral coverage:**

- **Weak or no assertion.** No `expect`, or only a broad check such as `toBeDefined`, `toBeTruthy`, `not.toThrow`, `toBeInstanceOf`, or `toBeGreaterThan(0)`. Some of these reject `undefined`, but can still accept an incorrect result. Assert the behavior that matters.
