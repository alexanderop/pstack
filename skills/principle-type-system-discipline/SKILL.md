...
- **Model expected failures as typed values.** Return `Result<T, E>` (or the language's equivalent) for recoverable failures, with discriminated error variants that describe what went wrong. Callers must handle or propagate failures explicitly; reserve exceptions for unexpected defects. In Effect-based code, use Effect's typed error channel. Adapt expected exceptions from external APIs at the boundary without disguising unexpected defects as ordinary failures. Reuse the project's existing result abstraction rather than adding a competing one. TypeScript exposes failures in the signature but does not prevent callers from ignoring the returned result.
...
- "Can this operation fail in a way the caller is expected to recover from?" Represent that failure in its return type or typed error channel, then handle or propagate it at each call site.
