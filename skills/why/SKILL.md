...
Before spawning investigators, list the MCP servers actually available in this session (`harness/built-ins.md` — every harness exposes the list differently). Use the available-tools map when present. Otherwise inspect the `mcps/` directory Cursor exposes for enabled MCP servers.
...
- `model`: your configured why-investigators model (harness default)
...
- `subagent_type`: the harness's general-purpose agent (`general-purpose` on Claude Code, `generalPurpose` on Cursor)
- `model`: your configured why-synthesizer model (harness default)
