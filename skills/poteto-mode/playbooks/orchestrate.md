...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the harness's subagent tool (`harness/<detected>.md`). State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3 where the harness allows it; check its subagent section before assuming a nested spawn has the full schema). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Detached from this machine wherever the harness supports it — Cursor cloud agents, and nothing equivalent elsewhere (see "long-running work" in `harness/<detected>.md`). Keep a worker local when the task needs this machine: runtime verification against a live UI or CLI; reading local transcripts; simulators and local IDE state; auth that exists only here. A detached agent cannot read the local store, so its brief inlines what it needs or points at repo paths. On a harness with no detached runner, every worker is local: give each its own worktree so they do not collide, and do not promise the user a run that survives the session. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
