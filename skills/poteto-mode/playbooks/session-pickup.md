...
1. Locate the prior trail. A local transcript, located per the "Transcripts" section of `harness/<detected>.md` and scoped to the active workspace (never glob across all workspaces, that reads private chats from unrelated projects), a background-run URL or output file, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
