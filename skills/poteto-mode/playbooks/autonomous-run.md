...
2. Pick the wake mechanism using your harness's loop or scheduler (`harness/built-ins.md`; `/loop` on Cursor and Claude Code, native wait tools on Codex, a polled background job on Copilot CLI — none of these are pstack skills). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
