...
**PRs.** Run `/deslop` from `cursor-team-kit` over the diff before commit, or pstack's own `/unslop` where that plugin does not exist (`harness/built-ins.md`). Run `/no-comments` before review. Write every PR title, PR description, and commit body with `/technical-writing`, then apply `/unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.
