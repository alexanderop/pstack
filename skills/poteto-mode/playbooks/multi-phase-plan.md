...
**Control skill.** Pick it by surface. Browser and web UIs use pstack **browse-web** with `agent-browser`; Electron web renderers use its app-owned CDP recipe. CLIs and TUIs use `control-cli` from `cursor-team-kit`. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
Each live lane runs on its own cloud VM at the PR head. Drive websites through pstack **browse-web** with `agent-browser`; use the matching CLI or native driver for other surfaces.
