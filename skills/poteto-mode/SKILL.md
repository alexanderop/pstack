...
- Website browsing, rendered-page inspection, browser interaction, screenshots, or live frontend verification → read **browse-web** (`../browse-web/SKILL.md`) and use `agent-browser`. This includes Electron web renderers with an app-owned CDP endpoint. Existing automated test suites keep their runners. For CLI/TUI work use the matching control skill (`control-cli` from `cursor-team-kit` where available); native IDE/desktop surfaces need their own driver. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
