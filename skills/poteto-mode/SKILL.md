...
**Before your first spawn, read the active harness file.** Read `harness/README.md`, run
`scripts/detect-harness.sh` from the pstack plugin root, and read the active
harness file. If detection returns `unknown`, follow the tool-interface fallback
in that README. It names the subagent tool, the model slugs this
...
- Website browsing, rendered-page inspection, browser interaction, screenshots, or live frontend verification → read **browse-web** (`../browse-web/SKILL.md`) and use its selected driver. This includes Electron; the `agent-browser` route requires an app-owned CDP endpoint for web renderers. Existing automated test suites keep their runners. For CLI/TUI work use the matching control skill (`control-cli` from `cursor-team-kit` where available); native IDE/desktop surfaces need their own driver. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
...
**Use the `poteto-agent` instructions for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `/poteto-mode` and `poteto-agent` route through the same wrapper. If the harness does not register that agent name, use its general-purpose agent and require it to read `agents/poteto-agent.md` from this plugin before work. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review. Respect what the skill prescribes, don't override to `poteto-agent`.

**Defaults for every subagent call.** Use the active harness file to map tool arguments and model roles. Backgrounded where supported, agent mode (readonly strips MCP), file pointers not inlined context, explicit model per role (configured via `/setup-pstack`, with defaults from the active harness file). Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model, whether the task needs judgment on vague intent or is a precisely specified sequence of steps to execute to the letter. Trivial mechanical edits go to your fast code model. Per-role lines in `~/.pstack/models.md` override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`). A role with no line keeps its default, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit the `model` argument).
