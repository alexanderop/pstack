#!/usr/bin/env bash
# Show what changed in upstream pstack since this fork was cut.
#
...
git clone --depth 1 --quiet https://github.com/cursor/plugins.git "$tmp/upstream"
...
# Files this fork owns outright. Drift in them is expected, not news.
exclude=(
	':(exclude)README.md'
	':(exclude).cursor-plugin/plugin.json'
	':(exclude)skills/setup-pstack/SKILL.md'
)
...
diff -ru \
	...
	--exclude='sync-upstream.sh' --exclude='detect-harness.sh' --exclude='validate.sh' \
